...
public class ScheduleUtils
{
    ...
    public static void createScheduleJob(Scheduler scheduler, SysJob job)
    {
        ...
        {
            // 构建job信息
            JobDetail jobDetail = JobBuilder.newJob(ScheduleJob.class).withIdentity(getJobKey(Convert.toLong(job.getJobId()))).build();
            ...
            CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity(getTriggerKey(Convert.toLong(job.getJobId())))
            ...
            {
                pauseJob(scheduler, Convert.toLong(job.getJobId()));
            }
        }
        catch (SchedulerException e)
        {
    ...
    public static void updateScheduleJob(Scheduler scheduler, SysJob job)
    {
        try
        {
            TriggerKey triggerKey = getTriggerKey(Convert.toLong(job.getJobId()));

            // 表达式调度构建器
            CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule(job.getCronExpression());
            cronScheduleBuilder = handleCronScheduleMisfirePolicy(job, cronScheduleBuilder);

            CronTrigger trigger = getCronTrigger(scheduler, Convert.toLong(job.getJobId()));
    ...
    public static int run(Scheduler scheduler, SysJob job)
    {
        ...
        try
        {
            ...
            scheduler.triggerJob(getJobKey(Convert.toLong(job.getJobId())), dataMap);
            rows = 1;
        }
        catch (SchedulerException e)
        {
