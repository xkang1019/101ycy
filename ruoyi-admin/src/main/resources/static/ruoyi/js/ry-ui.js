...
(function ($) {
    $.extend({
    	...
    	common: {
			 ...
			 * @author pmc
             */
            removeClassPref: function($e,pref){
				var allClass = $e.attr("class").toString().trim();
				var classs = allClass.split(/\s+/);//一个多多个空格切分
				for (var i = 0; i < classs.length ; i++) {
                    if (this.startWith(classs[i],pref)){
